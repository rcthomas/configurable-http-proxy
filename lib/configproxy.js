...
// The next 3 functions copied over from notebook JavaScript.
...
//      } else {
            ...
            var msg = _deserialize_array_buffer(data.buffer);   // changed data to data.buffer
            return msg;
//      }
    };
  ...
  handleProxy(kind, req, res) {
    ...
    return this.targetForReq(req).then(match => {
      ...
      req.on("data", function(chunk) {
        ...
        that.log.info("req on data");
        that.log.info(Object.keys(chunk));
        that.log.info(deserialize(chunk));
//      that.log.info(chunk);
      });

      res.on("data", function(chunk) {
        that.updateLastActivity(prefix);
        that.log.info("res on data");
        that.log.info(Object.keys(chunk));
        that.log.info(deserialize(chunk));
//      that.log.info(chunk);
      });
